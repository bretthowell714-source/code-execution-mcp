...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SOLARGYSTIX_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SOLARGYSTIX_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Solargystix API.
 */
export class Solargystix {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['SOLARGYSTIX_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SOLARGYSTIX_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.SolargystixError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Solargystix client with an apiKey option, like new Solargystix({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Solargystix.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SOLARGYSTIX_LOG'), "process.env['SOLARGYSTIX_LOG']", this) ??
      defaultLogLevel;
  ...
  static Solargystix = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static SolargystixError = Errors.SolargystixError;
...
Solargystix.Pets = Pets;
Solargystix.Store = Store;
Solargystix.Users = Users;

export declare namespace Solargystix {
